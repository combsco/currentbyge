defmodule CurrentByGE do
  use HTTPoison.Base
  alias CurrentByGE.Client

  @user_agent [{"User-agent", "currentbyge"}]
  ...
  defp extra_options do
    Application.get_env(:currentbyge, :request_options, [])
  end

  defp pagination(options) do
    Keyword.get(options, :pagination,
      Application.get_env(:currentbyge, :pagination, nil))
  end
  ...
  def authorization_header(%{access_token: token, predix_zone_id: predix_zone_id}, headers) do
